import { COUNT_VERTEX, ACCURACY, COUNT_MIDDLE_VERTEX, RADIUS_INTERACTION, SCALE_DEMO } from '../../constants/index'
...
export default class Cell {
  ...
  checkInteraction(cm) {
    if (Math.sqrt((cm.x - this.props.centerMass.x) * (cm.x - this.props.centerMass.x) +
        (cm.y - this.props.centerMass.y) * (cm.y - this.props.centerMass.y)) < RADIUS_INTERACTION * SCALE_DEMO) {
      return true;
    }
    return false;
  }
  ...
  create() {
    // console.log(this.pubSub.publish('getAverage'));
    this.calcCenterMass();
    this.calcSquare();
    this.calcRadiusCircle();
    this.calcRadiusCircleAnamorf();
    this.props.radInter = Math.ceil(this.props.values[this.props.active] / this.pubSub.publish('getAverage')) *
      Math.ceil(this.props.values[this.props.active] / this.pubSub.publish('getAverage')) + 5;
