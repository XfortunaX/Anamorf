...
export default class AnamorfProcess {
  ...
  setMatrix(matrix) {
    ...
    //   // let points = [];
    //   // let ps = {};
    //   // let r = c[this.thread.y * this.constants.size.width + this.thread.x].getRadius();
    //   let points = 0;
    //   let r = 50;
    //   for (let i1 = this.thread.x - r; i1 < this.thread.x + r; i1 += 1) {
    //     for (let j1 = this.thread.y - r; j1 < this.thread.y + r; j1 += 1) {
    //       if (this.thread.x + i1 > -1 && this.thread.x + i1 < 120 &&
    //         this.thread.y + j1 > -1 && this.thread.y + j1 < 105) {
    //           // points.push(c[this.thread.y * this.constants.size.width + this.thread.x]);
    //           points += 1;
    //           // points = c[i1 * this.constants.size.width + j1].getPoints();
    ...
    //   }
    //   // for (let n in ps) {
    //   //   ps[n].changeVector(c[this.thread.y * this.constants.size.width + this.thread.x].calcInteraction(ps[n]));
    //   // }
    //   return points;
    // }, {
    //   output: [120, 105]
